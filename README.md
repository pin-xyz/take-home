...
This repo contains a partially implemented note taking app using React-Router v7 / React. The goal for this take-home is to implement some of the missing features of this app.
...
This app is missing much of its core functionality. The task here is to finish implementing some of the missing features for an MVP version of our note taking app. Much of the work needed to complete this project will be done in `app/routes/index.tsx`.

### Finish home page

1. Update the `index.tsx` action functon to save notes to our Sqlite database. 
> Hint: you will need to import the prisma client from `.server/db.ts`
...
Great! Now that we can create notes, let's improve the user experience a little bit. We're primarily going to be focused on enhancing the UX using [useFetcher](https://reactrouter.com/api/hooks/useFetcher).
